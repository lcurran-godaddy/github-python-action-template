...
from src.utils import (
    get_input, set_output, log_info, log_warning, log_error,
    get_github_context, is_github_actions
)
...
class TestUtils:
    ...
    def test_get_input_with_required_input_missing(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ValueError, match="Required input 'test_input' is missing"):
    ...
    def test_set_output_without_github_output_file(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("builtins.print") as mock_print:
                set_output("test_output", "test_value")
                mock_print.assert_called_once_with("::set-output name=test_output::test_value")
    ...
    def test_is_github_actions_false(self):
        """Test is_github_actions returns False when GITHUB_ACTIONS is not set."""
        with patch.dict(os.environ, {}, clear=True):
            assert is_github_actions() is False
    
    def test_is_github_actions_false_when_set_to_false(self):
        """Test is_github_actions returns False when GITHUB_ACTIONS is set to false."""
